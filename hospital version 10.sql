...
select patientId,nhi,patientFname,patientLname,dob,gender,hte,floor(datediff(sysdate(),dob)/365.25) as 'Patient Age' from Patient;
...
select Patient.patientFname,Patient.patientLname 
from Patient
join Department
on floor(datediff(sysdate(),dob)/365.25) < '18' and 
Department.depname not like 'Paediatric Surgery';
